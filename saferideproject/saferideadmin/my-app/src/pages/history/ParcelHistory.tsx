            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 860 }}>
              <thead>
                <tr style={{ background: '#f8fafc', borderBottom: '1.5px solid #f1f5f9' }}>
                  {['#', 'Booking ID', 'Sender', 'Captain', 'Route', 'Sub Service', 'Fare', 'Date', 'Pickup / Delivery', 'Status', 'Actions'].map(h => (
              ...
              <tbody>
                ...
                {!loading && paginated.map((b, idx) => (
                  <tr key={b.id} onClick={() => setDetail(b)}
                    ...
                    <td style={{ padding: '11px 14px' }}>
                      {(b.pickup_otp_verified_at || b.delivered_at || b.completed_at) ? (
                        <div style={{ fontSize: 10, color: '#64748b', whiteSpace: 'nowrap' }}>
                          <div>Pickup: <b style={{ color: '#166534' }}>{fmtTime(b.pickup_otp_verified_at)}</b></div>
                          <div style={{ marginTop: 2 }}>Delivered: <b style={{ color: '#991b1b' }}>{fmtTime(b.delivered_at || b.completed_at)}</b></div>
                        </div>
