...
import {
  ...
  X, Eye, MapPin, Calendar, CheckCircle2, XCircle, Clock, Package,
} from 'lucide-react';
import { getParcelBookings } from '../../services/api';
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,0.5)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', backdropFilter: 'blur(4px)', padding: 16 }}>
      <div style={{ background: 'white', borderRadius: 20, width: 480, maxWidth: '95vw', maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 24px 60px rgba(0,0,0,0.18)', animation: 'slideUp 0.3s ease' }}>
        <div style={{ padding: '18px 22px', borderBottom: '1.5px solid #f1f5f9', display: 'flex', justifyContent: 'space-between', alignItems: 'center', position: 'sticky', top: 0, background: 'white', zIndex: 1 }}>
          <div>
            <h3 style={{ margin: 0, fontSize: 15, fontWeight: 700, color: '#0f172a' }}>Parcel Booking #{booking.id}</h3>
...
export default function ParcelHistory() {
  ...
  const fetchAll = useCallback(async () => {
    setLoading(true);
    try {
      const res  = await getParcelBookings({ limit: 1000 });
      ...
      setBookings(all);
    } catch (err) { console.error('[Parcel] fetch error:', err); setBookings([]); }
    ...
    <>
      ...
      <div style={{ padding: 24, animation: 'fadeSlideUp 0.4s ease' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20, flexWrap: 'wrap', gap: 12 }}>
          <div>
            <h2 style={{ fontSize: 18, fontWeight: 700, color: '#0f172a', margin: '0 0 2px' }}>Parcel History</h2>
            <p style={{ color: '#94a3b8', fontSize: 12, margin: 0 }}>All bookings for Parcel delivery service</p>
          </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(140px,1fr))', gap: 12, marginBottom: 20 }}>
          {[
            { label: 'Total',     value: stats.total,     bg: '#fff7ed', color: '#c2410c', icon: <Package size={18} color="#c2410c" /> },
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 860 }}>
              <thead>
                <tr style={{ background: '#f8fafc', borderBottom: '1.5px solid #f1f5f9' }}>
                  {['#', 'Booking ID', 'Sender', 'Captain', 'Route', 'Sub Service', 'Fare', 'Date', 'Pickup Time', 'Delivery Time', 'Status', 'Actions'].map(h => (
              ...
              <tbody>
                ...
                {!loading && paginated.length === 0 && (
                  <tr><td colSpan={12} style={{ padding: 48, textAlign: 'center' }}>
                    <Package size={32} color="#e2e8f0" style={{ display: 'block', margin: '0 auto 10px' }} />
                    <div style={{ color: '#94a3b8', fontSize: 13 }}>No Parcel bookings found.</div>
                  </td></tr>
                )}
                  ...
                  <tr key={b.id} onClick={() => setDetail(b)}
                    ...
                    <td style={{ padding: '11px 14px' }}>
                      {b.pickup_otp_verified_at
                        ? <span style={{ fontSize: 11, color: '#166534', fontWeight: 600, whiteSpace: 'nowrap' }}>{fmtDateTime(b.pickup_otp_verified_at)}</span>
                        : <span style={{ color: '#cbd5e1', fontSize: 11 }}>—</span>}
                    </td>
                    <td style={{ padding: '11px 14px' }}>
                      {(b.delivered_at || b.completed_at)
                        ? <span style={{ fontSize: 11, color: '#991b1b', fontWeight: 600, whiteSpace: 'nowrap' }}>{fmtDateTime(b.delivered_at || b.completed_at)}</span>
                        : <span style={{ color: '#cbd5e1', fontSize: 11 }}>—</span>}
                    </td>
                    <td style={{ padding: '11px 14px' }}><StatusBadge status={b.status} /></td>
                    <td style={{ padding: '11px 14px' }} onClick={e => e.stopPropagation()}>
                      <button onClick={() => setDetail(b)} style={{ background: '#fff7ed', border: 'none', color: '#ea580c', padding: 6, borderRadius: 8, cursor: 'pointer', display: 'flex' }}>
                        <Eye size={13} />
                      </button>
                    </td>
                  </tr>
