...
import {
  ...
  X, Eye, Users, Car, MapPin, Calendar, Navigation,
...
function TripsTab() {
  ...
  const fetchTrips = useCallback(async () => {
    ...
    try {
      ...
      console.log('[SelfSharing] extracted trips:', list.length);
      setTrips(list);
    } catch (err) { console.error('[SelfSharing] trips fetch error:', err); setTrips([]); }
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 800 }}>
            <thead>
              <tr style={{ background: '#f8fafc', borderBottom: '1.5px solid #f1f5f9' }}>
                {['#', 'Trip ID', 'Driver', 'Route', 'Seats', 'Fare', 'Schedule', 'Status', 'Actions'].map(h => (
            ...
            <tbody>
              ...
              {!loading && paginated.length === 0 && (
                <tr><td colSpan={9} style={{ padding: 48, textAlign: 'center', color: '#94a3b8', fontSize: 13 }}>No trips found.</td></tr>
              )}
...
function BookingsTab() {
  ...
  const fetchBookings = useCallback(async () => {
    ...
    try {
      ...
      console.log('[SelfSharing] extracted bookings:', list.length);
      setBookings(list);
    } catch (err) { console.error('[SelfSharing] bookings fetch error:', err); setBookings([]); }
    finally { setLoading(false); }
  }, []);
      ...
      <div style={{ background: 'white', borderRadius: 16, border: '1.5px solid #eef2f7', overflow: 'hidden' }}>
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 860 }}>
            <thead>
              <tr style={{ background: '#f8fafc', borderBottom: '1.5px solid #f1f5f9' }}>
                {['#', 'Booking ID', 'User', 'Driver', 'Route', 'Fare', 'Date', 'Status', 'Actions'].map(h => (
                  <th key={h} style={{ padding: '11px 14px', fontSize: 11, color: '#94a3b8', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.4px', whiteSpace: 'nowrap', textAlign: 'left' }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              ...
              {!loading && paginated.length === 0 && (
                <tr><td colSpan={9} style={{ padding: 48, textAlign: 'center', color: '#94a3b8', fontSize: 13 }}>No bookings found.</td></tr>
              )}
...
// ─── Main Export ───────────────────────────────────────────────────────────────
export default function SelfSharingHistory() {
  ...
  const stats = [
    { label: 'Module',  value: 'Self Sharing', bg: '#eef2ff', color: '#6366f1', icon: <Users size={18} color="#6366f1" /> },
    ...
    <>
      ...
      <div style={{ padding: 24, animation: 'fadeSlideUp 0.4s ease' }}>
        <div style={{ marginBottom: 20 }}>
          <h2 style={{ fontSize: 18, fontWeight: 700, color: '#0f172a', margin: '0 0 4px' }}>Self Sharing History</h2>
          <p style={{ color: '#94a3b8', fontSize: 12, margin: 0 }}>Manage self sharing trips and passenger bookings</p>
        </div>
